...
function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex w-[280px] flex-col bg-white border-r border-[#E2E8F0] h-[calc(100vh-64px)] sticky top-16">
...
function StatCard({ label, value, icon: Icon, color, trend, trendValue }: { label: string; value: string | number; icon: any; color: string; trend?: "up" | "down"; trendValue?: string | number }) {
    return (
        <div className="bg-white  p-4 rounded-[16px] border border-[#E2E8F0]  shadow-sm flex flex-col justify-between max-h-[120px]">
            ...
            <div>
                ...
                <p className="text-[11px] font-medium text-[#718096] truncate">{label}</p>
            </div>
        </div>
    );
}
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700 px-4 md:px-0">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5">
        <div>
            <h1 className="text-[28px] md:text-[36px] font-black tracking-tighter text-[#1A202C] ">Operational Hub</h1>
            <p className="text-[#718096] font-medium">Real-time logistics management</p>
        </div>
        <div className="grid grid-cols-1 min-[400px]:grid-cols-2 lg:flex w-full lg:w-auto gap-3">
            <button onClick={() => navigate('/admin/new-shipment')} className="h-[72px] px-4 bg-[#1B4332] text-white font-semibold rounded-[16px] shadow-sm active:scale-95 transition-all flex items-center gap-3 text-sm w-full justify-start lg:w-auto">
                <Plus className="w-4 h-4" /> New Parcel
            </button>
            <button onClick={() => navigate('/admin/sacks')} className="h-[72px] px-4 bg-white text-[#1B4332] border border-[#1B4332] font-semibold rounded-[16px] shadow-sm active:scale-95 transition-all flex items-center gap-3 text-sm w-full justify-start lg:w-auto">
                <Layers className="w-4 h-4" /> Build Sack
            </button>
            <button onClick={() => navigate('/admin/search')} className="h-[72px] px-4 border border-[#E2E8F0] text-[#4A5568] bg-white font-semibold rounded-[16px] shadow-sm active:scale-95 transition-all flex items-center gap-3 text-sm w-full justify-start lg:w-auto">
                <Search className="w-4 h-4" /> Search
            </button>
            <button onClick={() => navigate('/admin/shipments')} className="h-[72px] px-4 border border-[#E2E8F0] text-[#4A5568] bg-white font-semibold rounded-[16px] shadow-sm active:scale-95 transition-all flex items-center gap-3 text-sm w-full justify-start lg:w-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 bg-white  p-8 rounded-[3rem] border border-[#E2E8F0]  shadow-sm space-y-6">
           <div className="flex justify-between items-center">
              <div>
                 <h3 className="text-xl font-black text-[#1A202C]  uppercase tracking-tight">Revenue Dynamics</h3>
                 <p className="text-xs font-bold text-[#A0AEC0]">Weekly financial performance audit</p>
              </div>
              <div className="flex items-center gap-4">
                 <div className="text-right">
                    <p className="text-[10px] font-black text-[#A0AEC0] uppercase">Gross Yield</p>
        ...
        <div className="bg-[#0F172A] p-8 rounded-[3rem] text-white shadow-2xl space-y-8 flex flex-col justify-between overflow-hidden relative">
           ...
           <div className="relative z-10">
              ...
              <p className="text-[#A0AEC0] text-xs font-bold mb-8">Real-time terminal event log</p>
                 ...
                 {adminActions.slice(0, 5).map(log => (
                    <div key={log.id} className="flex gap-4 group">
                       ...
                       <div className="pb-4">
                          ...
                          <p className="text-[10px] text-[#A0AEC0] font-bold mb-1">{log.adminName} • {new Date(log.timestamp).toLocaleTimeString()}</p>
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                   <h1 className="text-3xl font-black tracking-tight text-[#1A202C]  uppercase">Security Oversight</h1>
                   <p className="text-[#A0AEC0] font-bold tracking-tight">Weight discrepancy detection & resolution terminal</p>
                </div>
                    ...
                    {['pending', 'resolved', 'ignored'].map(t => (
                        <button
                            ...
                            className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                                ...
                                : 'text-[#A0AEC0] hover:text-slate-600 dark:hover:text-white'
                            }`}
                        >
                            {t}
                        </button>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-white  border-b border-[#E2E8F0] ">
                            <tr className="text-[10px] font-black uppercase text-[#A0AEC0] tracking-[0.2em]">
                        ...
                        <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                            {filtered.map(a => (
                                <tr key={a.id} className="hover:bg-white/50 dark:hover:bg-white/5 transition-colors">
                                    <td className="px-10 py-8 font-mono font-black text-sm text-blue-600">
                                       <div className="flex gap-2"><span className="text-[10px] font-bold text-[#A0AEC0] uppercase tracking-widest">Secure Entry</span></div>
                                       {a.trackingNumber}
                                    </td>
                                    ...
                                    <td className="px-10 py-8 text-right">
                                        {tab === 'pending' ? (
                                            <div className="flex justify-end gap-3">
                                                <button onClick={() => handleAction(a.id, 'ignored')} className="h-12 px-6 border-2 border-slate-200 dark:border-white/10 text-[#A0AEC0] font-black uppercase text-[10px] rounded-xl hover:bg-slate-100 transition-all">Ignore</button>
                                                <button onClick={() => handleAction(a.id, 'resolved')} className="h-12 px-6 bg-[#1B4332] text-white font-black uppercase text-[10px] rounded-xl shadow-lg shadow-blue-600/20 active:scale-95 transition-all">Resolve Case</button>
                                            </div>
                                        ) : (
                                           <div className="text-right">
                                              <p className="text-[10px] font-black text-[#A0AEC0] uppercase mb-1">Final Decision Reason</p>
                                              <p className="text-xs font-bold text-slate-600 dark:text-[#A0AEC0] italic">"{a.reason}"</p>
                                           </div>
                                        )}
                                    </td>
                                </tr>
                    ...
                    {filtered.length === 0 && (
                       <div className="py-32 text-center space-y-4">
                          ...
                          <div>
                             ...
                             <p className="text-sm font-bold text-[#A0AEC0]">All inventory weights within tolerance levels</p>
                          </div>
                       </div>
                    )}
                </div>
...
function ShipmentDetailsModal({ shipment, onClose, onAction }: { shipment: Shipment, onClose: () => void, onAction: (type: string, data: any) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-[#0F172A]/95 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="bg-white  rounded-[3rem] w-full max-w-2xl overflow-hidden border border-white/10 shadow-2xl animate-in zoom-in-95 duration-500">
        <div className="p-10 border-b border-[#E2E8F0]  flex justify-between items-center bg-white/50 ">
            <div>
            ...
            <button onClick={onClose} className="p-4 bg-white  rounded-2xl shadow-sm hover:scale-110 active:scale-90 transition-all"><X className="w-6 h-6 text-[#A0AEC0]" /></button>
        </div>
        <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
           <div className="space-y-8">
              <div className="space-y-3">
                 <label className="text-[10px] font-black uppercase text-[#A0AEC0] tracking-widest ml-4">Terminal Status Override</label>
                    ...
                    {['received', 'shipped', 'arrived', 'delivered', 'on_hold', 'returned'].map(s => (
                        <button
                           ...
                           className={`px-4 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl border-2 transition-all ${
                              shipment.status === s
                              ? 'bg-[#1B4332] text-white border-blue-600 shadow-lg shadow-blue-600/20'
                              : 'bg-transparent border-[#E2E8F0]  text-[#A0AEC0] hover:border-blue-600/50'
                           }`}
                        >
                           {s.replace(/_/g, ' ')}
                        </button>
              ...
              <div className="space-y-3">
                 <label className="text-[10px] font-black uppercase text-[#A0AEC0] tracking-widest ml-4">Financial Correction ($)</label>
                 <div className="flex gap-3">
                    <input type="number" value={paidAmt} onChange={e => setPaidAmt(e.target.value)} className="flex-1 h-14 bg-white  border-2 border-transparent focus:border-blue-600 rounded-2xl px-6 font-black text-xl outline-none transition-all" />
                    <button onClick={() => { if(reason) onAction('adjust_balance', { paidAmount: parseFloat(paidAmt), reason }); else toast.error('Action Reason Mandatory'); }} className="h-14 px-8 bg-white text-[#1B4332] border border-[#1B4332] font-black uppercase text-xs tracking-widest rounded-2xl shadow-xl transition-all active:scale-95">Update</button>
                 </div>
              </div>
           </div>
           <div className="space-y-8">
              <div className="space-y-3">
                 <label className="text-[10px] font-black uppercase text-[#A0AEC0] tracking-widest ml-4">Audit Mandate (Required)</label>
                 <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Provide high-fidelity context for this terminal override..." className="w-full h-44 p-6 bg-white  border-2 border-transparent focus:border-blue-600 rounded-[2rem] text-sm font-bold outline-none transition-all resize-none" />
              </div>
...
function AllShipments() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
             <h1 className="text-3xl font-black tracking-tight text-[#1A202C]  uppercase">Inventory Control</h1>
             <p className="text-[#A0AEC0] font-bold tracking-tight">Active monitoring of all parcels within the ecosystem</p>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-80">
               ...
               <input type="text" value={filter} onChange={e => setFilter(e.target.value)} placeholder="Tracking ID, sender, receiver..." className="w-full h-14 pl-14 pr-6 bg-white  border-2 border-[#E2E8F0]  rounded-2xl font-bold text-sm focus:border-blue-600 outline-none transition-all shadow-sm" />
            </div>
          </div>
      </div>
      ...
      <div className="bg-white  border border-[#E2E8F0]  rounded-[3rem] overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
           <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                   {filtered.map((s) => (
                       <tr key={s.id} className="hover:bg-white/50 dark:hover:bg-white/5 transition-all group">
                           ...
                           <td className="px-10 py-8">
                              ...
                              <div className="flex flex-wrap gap-2 items-center">
                                 <span className="text-[10px] font-black text-[#A0AEC0] uppercase tracking-widest">{s.weight}kg • </span>
                           ...
                           <td className="px-10 py-8 text-right">
                              <button onClick={() => setSelectedShipment(s)} className="p-4 bg-white  rounded-2xl text-[#A0AEC0] group-hover:text-blue-600 group-hover:bg-blue-50 dark:group-hover:bg-blue-500/10 transition-all">
        ...
        {filtered.length === 0 && <div className="py-32 text-center text-[#A0AEC0] font-bold uppercase tracking-widest">No active protocols found</div>}
      </div>
...
function AuditLogs() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="flex justify-between items-center">
          <div>
             <h1 className="text-3xl font-black tracking-tight text-[#1A202C]  uppercase">Immutable Audit Trail</h1>
             <p className="text-[#A0AEC0] font-bold tracking-tight">Full-spectrum transparency of system overrides & adjustments</p>
          </div>
          <div className="relative w-80">
             <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-300" />
             <input placeholder="Search audit logs..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-14 pl-14 pr-6 bg-white  border-2 border-[#E2E8F0]  rounded-2xl font-bold text-sm focus:border-blue-600 outline-none transition-all shadow-sm" />
          </div>
      </div>
      ...
      <div className="bg-white  border border-[#E2E8F0]  rounded-[3rem] overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
           <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-slate-50 dark:divide-white/5 text-sm">
                   {filtered.map(log => (
                       <tr key={log.id} className="hover:bg-white/50 dark:hover:bg-white/5 transition-all">
                           <td className="px-10 py-8 font-black text-slate-800 dark:text-slate-200 uppercase tracking-tighter">{log.adminName}</td>
                           <td className="px-10 py-8">
                              <span className="px-3 py-1.5 bg-slate-100  rounded-xl text-[10px] font-black uppercase tracking-widest text-[#718096]">
                           ...
                           <td className="px-10 py-8 text-[#718096] font-bold italic text-xs leading-relaxed max-w-xs">"{log.reason}"</td>
                           <td className="px-10 py-8 text-right">
                              <p className="text-xs font-black text-[#1A202C]  uppercase mb-0.5">{new Date(log.timestamp).toLocaleDateString()}</p>
                              <p className="text-[10px] font-bold text-[#A0AEC0]">{new Date(log.timestamp).toLocaleTimeString()}</p>
                           </td>
                       </tr>
                   ))}
               </tbody>
        ...
        {filtered.length === 0 && <div className="py-32 text-center text-[#A0AEC0] font-bold uppercase tracking-widest">No audit data captured</div>}
      </div>
    </div>
  );
}
